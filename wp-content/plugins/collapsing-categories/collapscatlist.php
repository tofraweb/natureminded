<?php
/*
collapsing categories version: 2.1
    ...
    foreach ($posts as $post2) {
      ...
      if (!in_array($post2->ID, $postsToExclude)) {
        ...
        $posttext2.= "<li class='collapsing categories item" .  $self . 
            ...
            "' title='" . esc_attr( $title_text ) . "'>$linktext</a></li>\n";
      }
    }
  }
...
function get_sub_cat($cat, $categories, $parents, $posts,
  ...
  if (in_array($cat->term_id, $parents)) {
    foreach ($categories as $cat2) {
      ...
      $the_post = get_post();
      if ( in_array( $cat2->term_id, wp_get_post_categories( $the_post->ID ) ) ) {
        $parent.=" active";
      }
...
function list_categories($posts, $categories, $parents, $options) {
  ...
  foreach( $categories as $cat ) {
    ...
    $the_post = get_post();
    if ( in_array( $cat->term_id, wp_get_post_categories( $the_post->ID ) ) ) {
      $parent.=" active";
    }
